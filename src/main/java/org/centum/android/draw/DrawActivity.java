...
public class DrawActivity extends Activity implements View.OnClickListener {
    ...
    private final ImageView[] colorSquareViews = new ImageView[COLORS.length];
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        bgTextView = (TextView) findViewById(R.id.bg_textView);
        ...
        drawing.setBackgroundColor(getSharedPreferences("drawing", 0).getInt("bgColor", drawing.getBackgroundColor()));
    }
    ...
    private void onSave() {
        new AsyncTask<Void, Void, Void>() {
            ...
            private final ProgressDialog progressDialog = new ProgressDialog(DrawActivity.this);
            ...
            protected void onPreExecute() {
                ...
                handler.postDelayed(showDialogRunnable, 100);
            }
            ...
            protected Void doInBackground(Void... params) {
                ...
                if (bitmap == null) {
                    drawSurfaceView.buildDrawingCache(true);
                    bitmap = drawSurfaceView.getDrawingCache();
                }
    ...
    private Bitmap autoCropBitmap(Bitmap bitmap, Drawing drawing) {
        ...
        int x = Math.max(0, bounds.left - 10);
        int y = Math.max(0, bounds.top - 10);
        int width = Math.min(bounds.width() + 20, bitmap.getWidth() - x);
        int height = Math.min(bounds.height() + 20, bitmap.getHeight() - y);

        long maxMemory = Runtime.getRuntime().maxMemory();
        long usedMemory = Runtime.getRuntime().totalMemory();
        long freeMemory = maxMemory - usedMemory;
        long memoryLimit = freeMemory - 1048576 * 2; //2mb free

        //If there's enough memory, crop it, otherwise return original
        if (width * height * 4 <= memoryLimit &&
                width * height < bitmap.getWidth() * bitmap.getHeight()) {
            return Bitmap.createBitmap(bitmap, x, y, width, height);
        } else {
            //Toast.makeText(this, "Not enough memory to crop", Toast.LENGTH_SHORT).show();
            return bitmap;
        }
    }
    ...
    private void showBackgroundColorPicker() {
        final ColorPickerDialog colorPickerDialog = new ColorPickerDialog(this, drawing.getBackgroundColor());
        colorPickerDialog.setAlphaSliderVisible(true);
        colorPickerDialog.setTitle("Pick Color");
        colorPickerDialog.setButton(DialogInterface.BUTTON_POSITIVE, getString(android.R.string.ok), new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                customColor = colorPickerDialog.getColor();
                getSharedPreferences("drawing", 0).edit().putInt("bgColor", customColor).commit();
                drawing.setBackgroundColor(customColor);
