...
public class Stack implements Externalizable {
    ...
    private ArrayList<PlaySession> playSessions = new ArrayList<PlaySession>();
    private ArrayList<StackListener> listeners = new ArrayList<StackListener>();
    ...
    public PlaySession[] getPlaySessions() {
        return playSessions.toArray(new PlaySession[0]);
    }
    ...
    public void sortByWrong() {
        PlayStats playStats = getPlayStatsWithEnabledSessions();
        for (int i = 0; i < getNumberOfCards(); i++) {
            double max = -1;
            Card maxCard = null;
            for (int d = i; d < getNumberOfCards(); d++) {
                if (playStats.getPercentWrong(getCard(d)) > max) {
                    max = playStats.getPercentWrong(getCard(d));
                    maxCard = getCard(d);
                }
            }
            moveCard(maxCard, i);
    ...
    public void sortByCorrect() {
        ...
        for (int i = 0; i < getNumberOfCards(); i++) {
            ...
            Card maxCard = null;
            for (int d = i; d < getNumberOfCards(); d++) {
                if (playStats.getPercentCorrect(getCard(d)) > max) {
                    max = playStats.getPercentCorrect(getCard(d));
                    maxCard = getCard(d);
                }
            }
            moveCard(maxCard, i);
    ...
    public void sortByName() {
        ...
        Collections.sort(sortedCards, new Comparator<Card>() {
            ...
            public int compare(Card card, Card card2) {
                return card.getTitle().compareTo(card2.getTitle());
            }
        });

        for (int i = 0; i < getNumberOfCards(); i++) {
            moveCard(sortedCards.get(i), i);
        }
    }
