...
public class StacksDatabaseHelper extends SQLiteOpenHelper {
        ...
        if (!isSaving()) {
            ...
            new AsyncTask<Void, Void, Void>() {
                ...
                protected Void doInBackground(Void... voids) {
                    ...
                    for (Stack stack : StackManager.get().getArchivedStacks()) {
                        updateStack(stack);
                    }
    ...
    private boolean containsStackSqlID(long stackID) {
        for (Stack s : StackManager.get().getStacks()) {
            if (s.getSqlID() == stackID) {
                return true;
            }
        }
        for (Stack s : StackManager.get().getArchivedStacks()) {
            if (s.getSqlID() == stackID) {
                return true;
            }
        }
    ...
    private List<PlaySession> loadPlaySessionsForStack(Stack stack) {
        ...
        List<PlaySession> playSessions = new LinkedList<PlaySession>();
