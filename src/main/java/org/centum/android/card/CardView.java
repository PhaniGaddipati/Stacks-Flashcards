...
public class CardView extends RelativeLayout implements CardListener {
    ...
    private static final int COLOR_NONE = Color.parseColor("#9933CC");
    ...
    private void init() {
        ...
        editImageView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                new CardDialogFragment(stack, card).show(cardFragment.getFragmentManager(), "edit_card");
            }
        });
        ...
        try {
            titleTextSize = Integer.parseInt(PreferenceManager.getDefaultSharedPreferences(getContext()).getString(SettingsActivity.KEY_PREF_CARD_TITLE_SIZE, "18"));
        } catch (Exception e) {
            PreferenceManager.getDefaultSharedPreferences(getContext()).edit().putString(SettingsActivity.KEY_PREF_CARD_TITLE_SIZE, "18").commit();
        }
        try {
            detailsTextSize = Integer.parseInt(PreferenceManager.getDefaultSharedPreferences(getContext()).getString(SettingsActivity.KEY_PREF_CARD_DETAILS_SIZE, "14"));
        } catch (Exception e) {
            PreferenceManager.getDefaultSharedPreferences(getContext()).edit().putString(SettingsActivity.KEY_PREF_CARD_DETAILS_SIZE, "14").commit();
        }
